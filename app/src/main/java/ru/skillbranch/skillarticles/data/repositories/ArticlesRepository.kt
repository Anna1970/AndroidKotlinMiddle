...
object ArticlesRepository : IArticlesRepository {
    ...
    private val preference = PrefManager
    ...
    suspend fun addBookmark(articleId: String) {
        if (preference.accessToken.isEmpty()) return
        try {
            network.addBookmark(articleId, preference.accessToken)
        } catch (e:Throwable){
    ...
    suspend fun removeBookmark(articleId: String) {
        if (preference.accessToken.isEmpty()) return
        try {
            network.removeBookmark(articleId, preference.accessToken)
        } catch (e:Throwable){
            if (e is NoNetworkError) return
            throw e
        }
