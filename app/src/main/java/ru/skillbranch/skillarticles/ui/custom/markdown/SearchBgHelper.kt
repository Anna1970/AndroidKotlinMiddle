...
class SearchBgHelper (
    ...
    val drawableRights : Drawable by lazy {
        mockDrawable ?: GradientDrawable().apply {
            shape = GradientDrawable.RECTANGLE
            cornerRadii = floatArrayOf(
...
class MultiLineRender(
    ...
    override fun draw(
        ...
        for (line in startLine.inc() until endLine) {
            ...
            drawableMiddle.setBounds(layout.getLineLeft(line).toInt() - padding,
            lineTop,
    ...
    private fun drawEnd(
        ...
        drawableRights.setBounds(start, top, end, bottom)
        drawableRights.draw(canvas)
    }
}